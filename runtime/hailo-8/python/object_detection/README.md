...
Addition of validation support
------------------------------

It piggybacks on `object_detection.py`, adding detection data gathering and export to JSON, which is used to perform offline validation on a faster machine. There are two different flavors of the way the val metrics are computed, and they are chosen by a CLI argument `--metrics-version`:

| Option | Function Name | Implementation | Speed | Use When... |
| --- | --- | --- | --- | --- |
| `pyloop` | `calculate_and_print_metrics_table_pyloop` | Original Python loops  | Fastest for huge sets | You need max speed for dense classes |
| `numba_mproc` (default) | `calculate_and_print_metrics_table_acc_mp_numba` | Numba + multiprocessing | Fast for most cases | You want parallelism and maintainability |